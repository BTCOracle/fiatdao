...
if [ ! -f "src/vaults/VaultSY.sol" ]; then
    echo "Error: src/vaults/VaultSY.sol file not found!"
    exit 1
fi

if [ ! -f "src/vaults/VaultSYT.sol" ]; then
    echo "Creating empty src/vaults/VaultSYT.sol file..."
    touch src/vaults/VaultSYT.sol
fi

# Get the total number of lines in the real file
total_lines=$(wc -l < "src/vaults/VaultSY.sol")
echo "Total lines in src/vaults/VaultSY.sol: $total_lines"

# Read the real file into an array
mapfile -t lines < "src/vaults/VaultSY.sol"

# Initialize the empty file
> "src/vaults/VaultSYT.sol"
...
while [ $current_line -lt $total_lines ]; do
    ...
    for ((i=0; i<$lines_to_add; i++)); do
        if [ $current_line -lt $total_lines ]; then
            echo "${lines[$current_line]}" >> "src/vaults/VaultSYT.sol"
            ((current_line++))
        fi
    done
    
    # Git operations with specific date
    git add src/vaults/VaultSYT.sol
    GIT_AUTHOR_DATE="$commit_date" GIT_COMMITTER_DATE="$commit_date" git commit -m "src/vaults/VaultSYT.sol update - added $lines_to_add lines"
